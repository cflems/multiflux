...
else
{
	...
	if ($pun_user['is_admmod'])
	{
		...
		{
			$result_header = $db->query('SELECT 1 FROM '.$db->prefix.'reports WHERE zapped IS NULL'.' AND site_id='.SITE_ID) or error('Unable to fetch reports info', __FILE__, __LINE__, $db->error());
