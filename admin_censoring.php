...
if (isset($_POST['add_word']))
{
	...
	$db->query('INSERT INTO '.$db->prefix.'censoring (search_for, replace_with, site_id) VALUES (\''.$db->escape($search_for).'\', \''.$db->escape($replace_with).'\', '.SITE_ID.')') or error('Unable to add censor word', __FILE__, __LINE__, $db->error());
...
else if (isset($_POST['update']))
{
	...
	$db->query('UPDATE '.$db->prefix.'censoring SET search_for=\''.$db->escape($search_for).'\', replace_with=\''.$db->escape($replace_with).'\' WHERE id='.$id.' AND site_id='.SITE_ID) or error('Unable to update censor word', __FILE__, __LINE__, $db->error());
...
else if (isset($_POST['remove']))
{
	...
	$db->query('DELETE FROM '.$db->prefix.'censoring WHERE id='.$id.' AND site_id='.SITE_ID) or error('Unable to delete censor word', __FILE__, __LINE__, $db->error());
...
$result = $db->query('SELECT id, search_for, replace_with FROM '.$db->prefix.'censoring WHERE site_id='.SITE_ID.' ORDER BY id') or error('Unable to fetch censor word list', __FILE__, __LINE__, $db->error());
