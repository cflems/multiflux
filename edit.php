...
if (isset($_POST['form_sent']))
{
	...
	if (empty($errors) && !isset($_POST['preview']))
	{
		...
		if ($can_edit_subject)
		{
			...
			$db->query('UPDATE '.$db->prefix.'topics SET subject=\''.$db->escape($subject).'\', sticky='.$stick_topic.' WHERE (id='.$cur_post['tid'].' OR moved_to='.$cur_post['tid'].') AND site_id='.SITE_ID) or error('Unable to update topic', __FILE__, __LINE__, $db->error());
		...
		// Update the post
		$db->query('UPDATE '.$db->prefix.'posts SET message=\''.$db->escape($message).'\', hide_smilies='.$hide_smilies.$edited_sql.' WHERE id='.$id.' AND site_id='.SITE_ID) or error('Unable to update post', __FILE__, __LINE__, $db->error());
