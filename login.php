...
if (isset($_POST['form_sent']) && $action == 'in')
{
	...
	$result = $db->query('SELECT * FROM '.$db->prefix.'users WHERE '.$username_sql.' AND site_id='.SITE_ID) or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
	...
	if (!empty($cur_user['password']))
	{
			...
			if ($is_salt_authorized) // 1.3 used sha1(salt.sha1(pass))
 			{
				...
				$db->query('UPDATE '.$db->prefix.'users SET password=\''.$form_password_hash.'\', salt=NULL WHERE id='.$cur_user['id'].' AND site_id='.SITE_ID) or error('Unable to update user password', __FILE__, __LINE__, $db->error());
			}
		}
		...
		else if (strlen($cur_user['password']) != 40)
		{
			...
			if ($is_md5_authorized)
			{
				...
				$db->query('UPDATE '.$db->prefix.'users SET password=\''.$form_password_hash.'\' WHERE id='.$cur_user['id'].' AND site_id='.SITE_ID) or error('Unable to update user password', __FILE__, __LINE__, $db->error());
			}
		}
	...
	if (empty($errors))
	{
		...
		{
			$db->query('UPDATE '.$db->prefix.'users SET group_id='.$pun_config['o_default_user_group'].' WHERE id='.$cur_user['id'].' AND site_id='.SITE_ID) or error('Unable to update user status', __FILE__, __LINE__, $db->error());
		...
		$db->query('DELETE FROM '.$db->prefix.'online WHERE ident=\''.$db->escape(get_remote_address()).'\' AND site_id='.SITE_ID) or error('Unable to delete from online list', __FILE__, __LINE__, $db->error());
...
else if ($action == 'out')
{
	...
	$db->query('DELETE FROM '.$db->prefix.'online WHERE user_id='.$pun_user['id'].' AND site_id='.SITE_ID) or error('Unable to delete from online list', __FILE__, __LINE__, $db->error());
...
else if ($action == 'forget' || $action == 'forget_2')
{
	...
	if (isset($_POST['form_sent']))
	{
		...
		if (empty($errors))
		{
			$result = $db->query('SELECT id, username, last_email_sent FROM '.$db->prefix.'users WHERE email=\''.$db->escape($email).'\' AND site_id='.SITE_ID) or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
			...
			if ($db->num_rows($result))
			{
				...
				while ($cur_hit = $db->fetch_assoc($result))
				{
					...
					$db->query('UPDATE '.$db->prefix.'users SET activate_string=\''.pun_hash($new_password).'\', activate_key=\''.$new_password_key.'\', last_email_sent = '.time().' WHERE id='.$cur_hit['id'].' AND site_id='.SITE_ID) or error('Unable to update activation data', __FILE__, __LINE__, $db->error());
