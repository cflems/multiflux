...
if ($action == 'change_pass')
{
	if (isset($_GET['key']))
	{
		...
		$result = $db->query('SELECT * FROM '.$db->prefix.'users WHERE id='.$id.' AND site_id='.SITE_ID) or error('Unable to fetch new password', __FILE__, __LINE__, $db->error());
		...
		{
			$db->query('UPDATE '.$db->prefix.'users SET password=\''.$db->escape($cur_user['activate_string']).'\', activate_string=NULL, activate_key=NULL'.(!empty($cur_user['salt']) ? ', salt=NULL' : '').' WHERE id='.$id.' AND site_id='.SITE_ID) or error('Unable to update password', __FILE__, __LINE__, $db->error());
	...
	if ($pun_user['id'] != $id)
	{
		...
		{
			$result = $db->query('SELECT u.group_id, g.g_moderator FROM '.$db->prefix.'users AS u INNER JOIN '.$db->prefix.'groups AS g ON (g.g_id=u.group_id) WHERE u.id='.$id.' AND u.site_id='.SITE_ID) or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
	...
	if (isset($_POST['form_sent']))
	{
		...
		$result = $db->query('SELECT * FROM '.$db->prefix.'users WHERE id='.$id.' AND site_id='.SITE_ID) or error('Unable to fetch password', __FILE__, __LINE__, $db->error());
		$cur_user = $db->fetch_assoc($result);
		...
		$db->query('UPDATE '.$db->prefix.'users SET password=\''.$new_password_hash.'\''.(!empty($cur_user['salt']) ? ', salt=NULL' : '').' WHERE id='.$id.' AND site_id='.SITE_ID) or error('Unable to update password', __FILE__, __LINE__, $db->error());
...
else if ($action == 'change_email')
{
	...
	if (isset($_GET['key']))
	{
		...
		$result = $db->query('SELECT activate_string, activate_key FROM '.$db->prefix.'users WHERE id='.$id.' AND site_id='.SITE_ID) or error('Unable to fetch activation data', __FILE__, __LINE__, $db->error());
		...
		{
			$db->query('UPDATE '.$db->prefix.'users SET email=activate_string, activate_string=NULL, activate_key=NULL WHERE id='.$id.' AND site_id='.SITE_ID) or error('Unable to update email address', __FILE__, __LINE__, $db->error());
	...
	else if (isset($_POST['form_sent']))
	{
		...
		$result = $db->query('SELECT id, username FROM '.$db->prefix.'users WHERE email=\''.$db->escape($new_email).'\' AND site_id='.SITE_ID) or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
		if ($db->num_rows($result))
		{
		...
		$db->query('UPDATE '.$db->prefix.'users SET activate_string=\''.$db->escape($new_email).'\', activate_key=\''.$new_email_key.'\' WHERE id='.$id.' AND site_id='.SITE_ID) or error('Unable to update activation data', __FILE__, __LINE__, $db->error());
...
else if (isset($_POST['update_group_membership']))
{
	...
	$result = $db->query('SELECT group_id FROM '.$db->prefix.'users WHERE id='.$id.' AND site_id='.SITE_ID) or error('Unable to fetch user group', __FILE__, __LINE__, $db->error());
	$old_group_id = $db->result($result);

	$db->query('UPDATE '.$db->prefix.'users SET group_id='.$new_group_id.' WHERE id='.$id.' AND site_id='.SITE_ID) or error('Unable to change user group', __FILE__, __LINE__, $db->error());
	...
	$result = $db->query('SELECT g_moderator FROM '.$db->prefix.'groups WHERE g_id='.$new_group_id.' AND site_id='.SITE_ID) or error('Unable to fetch group', __FILE__, __LINE__, $db->error());
	$new_group_mod = $db->result($result);

	// If the user was a moderator or an administrator, we remove him/her from the moderator list in all forums as well
	if ($new_group_id != PUN_ADMIN && $new_group_mod != '1')
	{
		$result = $db->query('SELECT id, moderators FROM '.$db->prefix.'forums WHERE site_id='.SITE_ID) or error('Unable to fetch forum list', __FILE__, __LINE__, $db->error());
		...
		while ($cur_forum = $db->fetch_assoc($result))
		{
			...
			if (in_array($id, $cur_moderators))
			{
				...
				$db->query('UPDATE '.$db->prefix.'forums SET moderators='.$cur_moderators.' WHERE id='.$cur_forum['id'].' AND site_id='.SITE_ID) or error('Unable to update forum', __FILE__, __LINE__, $db->error());
			}
		}
	}
...
else if (isset($_POST['update_forums']))
{
	...
	$result = $db->query('SELECT username FROM '.$db->prefix.'users WHERE id='.$id.' AND site_id='.SITE_ID) or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
	...
	while ($cur_forum = $db->fetch_assoc($result))
	{
		...
		if (in_array($cur_forum['id'], $moderator_in) && !in_array($id, $cur_moderators))
		{
			...
			$db->query('UPDATE '.$db->prefix.'forums SET moderators=\''.$db->escape(serialize($cur_moderators)).'\' WHERE id='.$cur_forum['id'].' AND site_id='.SITE_ID) or error('Unable to update forum', __FILE__, __LINE__, $db->error());
		}
...
else if (isset($_POST['ban']))
{
	...
	$result = $db->query('SELECT username FROM '.$db->prefix.'users WHERE id='.$id.' AND site_id='.SITE_ID) or error('Unable to fetch username', __FILE__, __LINE__, $db->error());
	$username = $db->result($result);

	// Check whether user is already banned
	$result = $db->query('SELECT id FROM '.$db->prefix.'bans WHERE username = \''.$db->escape($username).'\' AND site_id='.SITE_ID.' ORDER BY expire IS NULL DESC, expire DESC LIMIT 1') or error('Unable to fetch ban ID', __FILE__, __LINE__, $db->error());
	if ($db->num_rows($result))
	{
...
else if ($action == 'promote')
{
	...
	$sql = 'SELECT g.g_promote_next_group FROM '.$db->prefix.'groups AS g INNER JOIN '.$db->prefix.'users AS u ON u.group_id=g.g_id WHERE u.id='.$id.' AND g.g_promote_next_group>0 AND u.site_id='.SITE_ID;
	...
	$db->query('UPDATE '.$db->prefix.'users SET group_id='.$next_group_id.' WHERE id='.$id.' AND site_id='.SITE_ID) or error('Unable to promote user', __FILE__, __LINE__, $db->error());
...
else if (isset($_POST['delete_user']) || isset($_POST['delete_user_comply']))
{
	...
	$result = $db->query('SELECT group_id, username FROM '.$db->prefix.'users WHERE id='.$id.' AND site_id='.SITE_ID) or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
	...
	if (isset($_POST['delete_user_comply']))
	{
		...
		$result = $db->query('SELECT g_moderator FROM '.$db->prefix.'groups WHERE g_id='.$group_id.' AND site_id='.SITE_ID) or error('Unable to fetch group', __FILE__, __LINE__, $db->error());
		...
		$db->query('DELETE FROM '.$db->prefix.'topic_subscriptions WHERE user_id='.$id.' AND site_id='.SITE_ID) or error('Unable to delete topic subscriptions', __FILE__, __LINE__, $db->error());
		$db->query('DELETE FROM '.$db->prefix.'forum_subscriptions WHERE user_id='.$id.' AND site_id='.SITE_ID) or error('Unable to delete forum subscriptions', __FILE__, __LINE__, $db->error());

		// Remove him/her from the online list (if they happen to be logged in)
		$db->query('DELETE FROM '.$db->prefix.'online WHERE user_id='.$id.' AND site_id='.SITE_ID) or error('Unable to remove user from online list', __FILE__, __LINE__, $db->error());
		...
		if (isset($_POST['delete_posts']))
		{
			...
			$result = $db->query('SELECT p.id, p.topic_id, t.forum_id FROM '.$db->prefix.'posts AS p INNER JOIN '.$db->prefix.'topics AS t ON t.id=p.topic_id INNER JOIN '.$db->prefix.'forums AS f ON f.id=t.forum_id WHERE p.poster_id='.$id.' AND p.site_id='.SITE_ID) or error('Unable to fetch posts', __FILE__, __LINE__, $db->error());
			if ($db->num_rows($result))
			{
				while ($cur_post = $db->fetch_assoc($result))
				{
					// Determine whether this post is the "topic post" or not
					$result2 = $db->query('SELECT id FROM '.$db->prefix.'posts WHERE topic_id='.$cur_post['topic_id'].' AND site_id='.SITE_ID.' ORDER BY posted LIMIT 1') or error('Unable to fetch post info', __FILE__, __LINE__, $db->error());
		...
		// Delete the user
		$db->query('DELETE FROM '.$db->prefix.'users WHERE id='.$id.' AND site_id='.SITE_ID) or error('Unable to delete user', __FILE__, __LINE__, $db->error());
...
else if (isset($_POST['form_sent']))
{
	...
	$result = $db->query('SELECT u.username, u.group_id, g.g_moderator FROM '.$db->prefix.'users AS u LEFT JOIN '.$db->prefix.'groups AS g ON (g.g_id=u.group_id) WHERE u.id='.$id.' AND u.site_id='.SITE_ID) or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
	if (!$db->num_rows($result))
		message($lang_common['Bad request'], false, '404 Not Found');
	...
	$db->query('UPDATE '.$db->prefix.'users SET '.implode(',', $temp).' WHERE id='.$id.' AND site_id='.SITE_ID) or error('Unable to update profile', __FILE__, __LINE__, $db->error());

	// If we changed the username we have to update some stuff
	if ($username_updated)
	{
		$db->query('UPDATE '.$db->prefix.'bans SET username=\''.$db->escape($form['username']).'\' WHERE username=\''.$db->escape($old_username).'\' AND site_id='.SITE_ID) or error('Unable to update bans', __FILE__, __LINE__, $db->error());
		// If any bans were updated, we will need to know because the cache will need to be regenerated.
		if ($db->affected_rows() > 0)
			$bans_updated = true;
		$db->query('UPDATE '.$db->prefix.'posts SET poster=\''.$db->escape($form['username']).'\' WHERE poster_id='.$id.' AND site_id='.SITE_ID) or error('Unable to update posts', __FILE__, __LINE__, $db->error());
		$db->query('UPDATE '.$db->prefix.'posts SET edited_by=\''.$db->escape($form['username']).'\' WHERE edited_by=\''.$db->escape($old_username).'\' AND site_id='.SITE_ID) or error('Unable to update posts', __FILE__, __LINE__, $db->error());
		$db->query('UPDATE '.$db->prefix.'topics SET poster=\''.$db->escape($form['username']).'\' WHERE poster=\''.$db->escape($old_username).'\' AND site_id='.SITE_ID) or error('Unable to update topics', __FILE__, __LINE__, $db->error());
		$db->query('UPDATE '.$db->prefix.'topics SET last_poster=\''.$db->escape($form['username']).'\' WHERE last_poster=\''.$db->escape($old_username).'\' AND site_id='.SITE_ID) or error('Unable to update topics', __FILE__, __LINE__, $db->error());
		$db->query('UPDATE '.$db->prefix.'forums SET last_poster=\''.$db->escape($form['username']).'\' WHERE last_poster=\''.$db->escape($old_username).'\' AND site_id='.SITE_ID) or error('Unable to update forums', __FILE__, __LINE__, $db->error());
		$db->query('UPDATE '.$db->prefix.'online SET ident=\''.$db->escape($form['username']).'\' WHERE ident=\''.$db->escape($old_username).'\' AND site_id='.SITE_ID) or error('Unable to update online list', __FILE__, __LINE__, $db->error());

		// If the user is a moderator or an administrator we have to update the moderator lists
		$result = $db->query('SELECT group_id FROM '.$db->prefix.'users WHERE id='.$id.' AND site_id='.SITE_ID) or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
...
$result = $db->query('SELECT u.username, u.email, u.title, u.realname, u.url, u.jabber, u.icq, u.msn, u.aim, u.yahoo, u.location, u.signature, u.disp_topics, u.disp_posts, u.email_setting, u.notify_with_post, u.auto_notify, u.show_smilies, u.show_img, u.show_img_sig, u.show_avatars, u.show_sig, u.timezone, u.dst, u.language, u.style, u.num_posts, u.last_post, u.registered, u.registration_ip, u.admin_note, u.date_format, u.time_format, u.last_visit, g.g_id, g.g_user_title, g.g_moderator FROM '.$db->prefix.'users AS u LEFT JOIN '.$db->prefix.'groups AS g ON g.g_id=u.group_id WHERE u.id='.$id.' AND u.site_id='.SITE_ID) or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
if (!$db->num_rows($result))
	message($lang_common['Bad request'], false, '404 Not Found');
...
<?php
				...
				$result = $db->query('SELECT g_id, g_title FROM '.$db->prefix.'groups WHERE g_id!='.PUN_GUEST.' AND site_id='.SITE_ID.' ORDER BY g_title') or error('Unable to fetch user group list', __FILE__, __LINE__, $db->error());
...
<?php
				...
				$result = $db->query('SELECT c.id AS cid, c.cat_name, f.id AS fid, f.forum_name, f.moderators FROM '.$db->prefix.'categories AS c INNER JOIN '.$db->prefix.'forums AS f ON c.id=f.cat_id WHERE f.redirect_url IS NULL AND c.site_id='.SITE_ID.' ORDER BY c.disp_position, c.id, f.disp_position') or error('Unable to fetch category/forum list', __FILE__, __LINE__, $db->error());
