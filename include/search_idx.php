...
function update_search_index($mode, $post_id, $message, $subject = null)
{
	...
	if ($mode == 'edit')
	{
		$result = $db->query('SELECT w.id, w.word, m.subject_match FROM '.$db->prefix.'search_words AS w INNER JOIN '.$db->prefix.'search_matches AS m ON w.id=m.word_id WHERE m.post_id='.$post_id.' AND w.site_id='.SITE_ID, true) or error('Unable to fetch search index words', __FILE__, __LINE__, $db->error());
	...
	{
		$result = $db->query('SELECT id, word FROM '.$db->prefix.'search_words WHERE word IN(\''.implode('\',\'', array_map(array($db, 'escape'), $unique_words)).'\') AND site_id='.SITE_ID, true) or error('Unable to fetch search index words', __FILE__, __LINE__, $db->error());
		...
		if (!empty($new_words))
		{
			switch ($db_type)
			{
				...
				case 'mysql_innodb':
				case 'mysqli_innodb':
					$db->query('INSERT INTO '.$db->prefix.'search_words (word, site_id) VALUES(\''.implode('\', '.SITE_ID.'),(\'', array_map(array($db, 'escape'), $new_words)).'\', '.SITE_ID.')');
					break;

				default:
					foreach ($new_words as $word)
						$db->query('INSERT INTO '.$db->prefix.'search_words (word, site_id) VALUES(\''.$db->escape($word).'\', '.SITE_ID.')');
					break;
			}
		}
	...
	foreach ($words['del'] as $match_in => $wordlist)
	{
		...
		if (!empty($wordlist))
		{
			...
			$db->query('DELETE FROM '.$db->prefix.'search_matches WHERE word_id IN('.$sql.') AND post_id='.$post_id.' AND subject_match='.$subject_match.' AND site_id='.SITE_ID) or error('Unable to delete search index word matches', __FILE__, __LINE__, $db->error());
		}
	}
	...
	foreach ($words['add'] as $match_in => $wordlist)
	{
		...
		if (!empty($wordlist))
			$db->query('INSERT INTO '.$db->prefix.'search_matches (post_id, word_id, subject_match, site_id) SELECT '.$post_id.', id, '.$subject_match.', site_id FROM '.$db->prefix.'search_words WHERE word IN(\''.implode('\',\'', array_map(array($db, 'escape'), $wordlist)).'\') AND site_id='.SITE_ID) or error('Unable to insert search index word matches', __FILE__, __LINE__, $db->error());
	}
...
function strip_search_index($post_ids)
{
	...
	switch ($db_type)
	{
		...
		case 'mysqli_innodb':
		{
			$result = $db->query('SELECT word_id FROM '.$db->prefix.'search_matches WHERE post_id IN('.$post_ids.') AND site_id='.SITE_ID.' GROUP BY word_id') or error('Unable to fetch search index word match', __FILE__, __LINE__, $db->error());
			...
			if ($db->num_rows($result))
			{
				...
				$result = $db->query('SELECT word_id FROM '.$db->prefix.'search_matches WHERE word_id IN('.$word_ids.') AND site_id='.SITE_ID.' GROUP BY word_id HAVING COUNT(word_id)=1') or error('Unable to fetch search index word match', __FILE__, __LINE__, $db->error());
				...
				if ($db->num_rows($result))
				{
					...
					$db->query('DELETE FROM '.$db->prefix.'search_words WHERE id IN('.$word_ids.') AND site_id='.SITE_ID) or error('Unable to delete search index word', __FILE__, __LINE__, $db->error());
				}
			}
